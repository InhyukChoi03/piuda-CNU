...
model_path="$model_root/ggml-tiny.bin"
...
download_verified "$model_url" "$temporary_dir/ggml-tiny.bin" "$model_sha256"
...
install -m 0644 "$temporary_dir/ggml-tiny.bin" "$model_path"
install -m 0644 "$temporary_dir/ggml-silero-v6.2.0.bin" "$vad_path"

echo "로컬 한국어 음성인식 설치 완료: whisper.cpp $whisper_version + tiny + Silero VAD"
