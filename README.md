...
- Raspberry Pi USB 마이크와 `whisper.cpp` tiny·Silero VAD 기반 오프라인 한국어 STT, 로컬 음성 출력 및 iOS 음성 입출력
...
음성 질문을 사용하려면 Raspberry Pi에 USB 마이크 또는 마이크가 있는 USB 헤드셋이 필요합니다. Raspberry Pi 5 본체에는 내장 마이크가 없습니다. `install.sh`가 Raspberry Pi ARM64용 `whisper.cpp` 실행 파일과 tiny 다국어 모델·Silero VAD 모델을 검증된 체크섬으로 설치합니다. 키오스크의 **말하기** 버튼을 누르면 USB 마이크를 5초간 직접 녹음하고 Pi 안에서 한국어 텍스트로 변환하므로 핫스팟에 인터넷 연결이 없어도 동작합니다. 텍스트 질문과 보호자 알림은 마이크 없이도 동작합니다.
